...
function pF = GetSigFeatures_tmabsTr(pF)
% 2018-09-21 / Creation
    L = length(pF.absdata);
    pF.f.tmabsTr1 = mean(pF.absdata(1:round(0.5*L),:));  
    pF.f.tmabsTr2 = mean(pF.absdata(round(0.25*L)+1:round(0.75*L),:));  
    pF.f.tmabsTr3 = mean(pF.absdata(round(0.5*L)+1:end,:));  
end

% ------------------------------------------------
function pF = GetSigFeatures_tmax(pF)
    pF.f.tmax = max(pF.data);
end

function pF = GetSigFeatures_tmin(pF)
    pF.f.tmin = min(pF.data);
end

% ----- Features realted to magnet data -----------------------------------

function pF = GetSigFeatures_mslp(pF)
% Slope of the window
    pF.f.mslp = pF.data(end,:) - pF.data(1,:);
end

function pF  = GetSigFeatures_mlv(pF)
% Last value of the window
    pF.f.mlv = pF.data(end,:);
end

% ------------------------------------------------
function pF = GetSigFeatures_tar(pF)
    for ch = 1:size(pF.data,2)
        coeff = aryule(pF.data(:,ch),6);
        pF.f.tar1(ch) = coeff(2);
        pF.f.tar2(ch) = coeff(3);
        pF.f.tar3(ch) = coeff(4);
        pF.f.tar4(ch) = coeff(5);
        pF.f.tar5(ch) = coeff(6);
        pF.f.tar6(ch) = coeff(7);
    end
end

% -----------------------------------------------
function pF = GetSigFeatures_twlTr(pF)
% Waveform Length (acumulative changes in the length)
% 2011-07-27 Max Ortiz / Creation 
    L = round(length(pF.data)/2);
    pF.f.twlTr1 = sum(abs(pF.data(2:L,:) - pF.data(1:(L-1),:)));
    pF.f.twlTr2 = sum(abs(pF.data((L/2+2):1.5*L,:) - pF.data((L/2+1):(1.5*L-1),:)));
    pF.f.twlTr3 = sum(abs(pF.data((L+2):end,:) - pF.data((L+1):(end-1),:)));
end

% ------------------------------------------------
function pF = GetSigFeatures_trmsTr(pF)
    L = round(length(pF.absdata)/2);
    pF.f.trmsTr1 = sqrt(sum(pF.absdata(1:L,:).^2)/L);
    pF.f.trmsTr2 = sqrt(sum(pF.absdata((L/2 + 1):1.5*L,:).^2)/L);
    pF.f.trmsTr3 = sqrt(sum(pF.absdata((L+1):end,:).^2)/L);
end

% ------------------------------------------------
function pF = GetSigFeatures_tzcTr(pF)
% 2011-07-27 Max Ortiz / Creation
    %check if tmabs is available
    if ~isfield(pF.f,'tmabsTr')
        pF = GetSigFeatures_tmabsTr(pF);
    end
    L = round(size(pF.data,1)/2);
    tmp1 = repmat(pF.f.tmabsTr1,[L,1] );
    tmp2 = repmat(pF.f.tmabsTr2,[L,1] );
    tmp3 = repmat(pF.f.tmabsTr3,[L,1] );
    zc1 = ( pF.data(1:L,:) >= tmp1 ) - (pF.data(1:L,:) < tmp1 );
    zc2 = ( pF.data((0.5*L+1):1.5*L,:) >= tmp2 ) - (pF.data((0.5*L+1):1.5*L,:) < tmp2 );
    zc3 = ( pF.data((L+1):end,:) >= tmp3 ) - (pF.data((L+1):end,:) < tmp3 );
    pF.f.tzcTr1 = sum( ( zc1(1:L-1,:) - zc1(2:L,:) ) ~= 0);
    pF.f.tzcTr2 = sum( ( zc2(1:L-1,:) - zc2(2:L,:) ) ~= 0);
    pF.f.tzcTr3 = sum( ( zc3(1:L-1,:) - zc3(2:L,:) ) ~= 0);
end

function pF = GetSigFeatures_tslpch2Tr(pF)

    if ~isfield(pF,'diffData')
        pF.diffData = diff(pF.data);           % Get the diff
    end    
    L = round(length(pF.data)/2);
    zc = (pF.diffData(1:(L-1),:) > 0 ) - (pF.diffData(1:(L-1),:) < 0);
    pF.f.tslpch2Tr1 = sum( abs( zc(1:end-1,:) - zc(2:end,:) ) > 1 );
    zc = (pF.diffData((0.5*L+1):(1.5*L-1),:) > 0 ) - (pF.diffData((0.5*L+1):(1.5*L-1),:) < 0);
    pF.f.tslpch2Tr2 = sum( abs( zc(1:end-1,:) - zc(2:end,:) ) > 1 );
    zc = (pF.diffData((L+1):end,:) > 0 ) - (pF.diffData((L+1):end,:) < 0);
    pF.f.tslpch2Tr3 = sum( abs( zc(1:end-1,:) - zc(2:end,:) ) > 1 );
end
