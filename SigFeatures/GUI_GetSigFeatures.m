...
function varargout = GUI_GetSigFeatures(varargin)
% GUI_GETSIGFEATURES M-file for GUI_GetSigFeatures.fig
%      GUI_GETSIGFEATURES, by itself, creates a new GUI_GETSIGFEATURES or raises the existing
%      singleton*.
%
%      H = GUI_GETSIGFEATURES returns the handle to a new GUI_GETSIGFEATURES or the handle to
%      the existing singleton*.
%
%      GUI_GETSIGFEATURES('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in GUI_GETSIGFEATURES.M with the given input arguments.
%
%      GUI_GETSIGFEATURES('Property','Value',...) creates a new GUI_GETSIGFEATURES or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before GUI_GetSigFeatures_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to GUI_GetSigFeatures_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help GUI_GetSigFeatures

% Last Modified by GUIDE v2.5 14-Jun-2011 18:28:52

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @GUI_GetSigFeatures_OpeningFcn, ...
                   'gui_OutputFcn',  @GUI_GetSigFeatures_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before GUI_GetSigFeatures is made visible.
function GUI_GetSigFeatures_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to GUI_GetSigFeatures (see VARARGIN)

% Choose default command line output for GUI_GetSigFeatures
handles.output = hObject;

% Update handles structure
guidata(hObject, handles);

% UIWAIT makes GUI_GetSigFeatures wait for user response (see UIRESUME)
% uiwait(handles.figure1);


% --- Outputs from this function are returned to the command line.
function varargout = GUI_GetSigFeatures_OutputFcn(hObject, eventdata, handles) 
...
function pb_treat_Callback(hObject, eventdata, handles)
    ...
    treated_data.mov = get(phandles.lb_movements,'String');
    % Save the treated data into a "text" object in the parent figure
    set(phandles.t_treated_data,'UserData',treated_data);    
    % Transfer the number of sets
    set(phandles.et_trSets,'String',num2str(treated_data.trSets));    
    set(phandles.et_vSets,'String',num2str(treated_data.vSets));    
    set(phandles.et_tSets,'String',num2str(treated_data.tSets));    
