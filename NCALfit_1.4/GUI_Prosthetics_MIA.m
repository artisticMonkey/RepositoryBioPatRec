...
% Last Modified by GUIDE v2.5 08-Sep-2020 19:10:46
...
% handles    structure with handles and user data (see GUIDATA)
    ...
    % Start simulated control routine
    fwrite(obj,'q','char');
    replay = char(fread(obj,1,'char'));
    if strcmp(replay,'q')
        set(handles.t_msg,'String','Grasp-Test Start');
    else
        set(handles.t_msg,'String','Error Start'); 
        fclose(obj);
        return
    end
    fwrite(obj,samples,'uint16');
    s0 = [];
    s1 = [];
    s2 = [];
    s3 = [];
    s4 = [];
    greenSize = 40;
    for iteration = 1:samples
        MAV = round(get(handles.sl_simulatedMAV,'Value'));
        mov = get(handles.pm_movement,'Value') - 1;
        fwrite(obj,MAV,'uint8');
        switch mov
            case 0 % MIA_REST
                fwrite(obj,0,'uint8');
            case 1 % MIA_OPEN
                fwrite(obj,5,'uint8');
            case 2 % MIA_CYLINDRICAL_GRASP
                fwrite(obj,3,'uint8');
            case 3 % MIA_BIDIGIT_GRASP
                fwrite(obj,4,'uint8');
            case 4 % MIA_LATERAL_GRASP
                fwrite(obj,6,'uint8');     
        end
               
        % Read the predicted movement from Mia
        % Movements are hardcoded accordingly to this definition:
        % #define REST 			  		0
        % #define OPENHAND 			  	5
        % #define CYLINDRICAL GRASP  	3
        % #define BIDIGIT GRASTP        4	
        % #define LATERAL GRASP         6
...
%                 set(handles.t_msg,'String','extend');
        ...
        drawnow;
        outMov(iteration,:) = [movIdx activLevel rtdStep rtPWM];
        
        %simulate control routine frequency
    end
    % End simulated control routine
    replay = char(fread(obj,1,'char'));
    if strcmp(replay,'q')
        set(handles.t_msg,'String','Grasp-Test End');
    else
        set(handles.t_msg,'String','Error Ending'); 
        fclose(obj);
        return
    end
    % Close connection
...
%         fread(handles.obj,1,'uint32');
%         fread(handles.obj,1,'char');  % '+' character DD 20190712
        movIdx = fread(obj,1,'char');   
        activLevel = fread(obj,1,'char'); % MAV
        rtStrength = fread(obj,1,'char'); % Strength
        rtdStep = fread(obj,1,'char');
        rtPWM = fread(obj,1,'char');
        set(handles.txt_predict,'String',num2str(movIdx));
        set(handles.txt_activLevel,'String',num2str(activLevel)); % activLevel for MIA hand
        delete(s0);
        delete(s1);
        delete(s2);
        delete(s3);
        ...
        switch movIdx
            ...
            case 6
                set(handles.t_msg,'String','Lateral');
            case 7 
                set(handles.t_msg,'String','Wrist Extention');
            case 8 
                set(handles.t_msg,'String','Wrist Flexion');
%             case 4
%                 set(handles.t_msg,'String','flex');
%             case 5
%                 set(handles.t_msg,'String','extend');
        ...
        drawnow;
        outMov(i,:) = [movIdx activLevel rtdStep rtPWM];
        if go == 0
            break;
        end
    end
...
% handles    structure with handles and user data (see GUIDATA)
    ...
    fopen(obj);
    % read settings
    fwrite(obj,hex2dec('23'),'char');
    replay = fread(obj,1,'char');
    if replay == hex2dec('23')
        ...
        replay = char(fread(obj,1,'char'));
        if replay == hex2dec('23')
            status = sprintf('MIAhand = %s\nMotors = %s\nPost-Control-Algorithm = %s\nramp length = %d\nramp mode = %d\nmajority vote steps = %d\nsensitivity = %d\nhand minimum speed = %d\nhand maximum speed = %d\nminModSpeedStep = %d\nmaxModSpeedStep = %d\nshpPosMinPwm = %d\nshpPosMaxPwm = %d\nminActThreshold = %d\n', SH, enableM, PC, rampLength, rampMode, mvSteps, sensitivity, min, max, minModSpeedStep, maxModSpeedStep, shpPosMinPwm, shpPosMaxPwm, minThresh);
...
function sl_simulatedMAV_Callback(hObject, eventdata, handles)
% hObject    handle to sl_simulatedMAV (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
...
% --- Executes during object creation, after setting all properties.
function sl_simulatedMAV_CreateFcn(hObject, eventdata, handles)
% hObject    handle to sl_simulatedMAV (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    empty - handles not created until after all CreateFcns called
...
% --- Executes on button press in cb_MIAEnable.
function cb_MIAEnable_Callback(hObject, eventdata, handles)
% hObject    handle to cb_MIAEnable (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

% Hint: get(hObject,'Value') returns toggle state of cb_MIAEnable
    % Get connection object
    if isfield(handles,'obj')
        obj = handles.obj;
    else
        set(handles.t_msg,'String','No connection obj found');   
        if MIAEnable
            set(handles.cb_MIAEnable,'Value',0);
        else
            set(handles.cb_MIAEnable,'Value',1);
        end
        return;
    end
    MIAEnable = get(handles.cb_MIAEnable,'Value');
    % Open the connection
    fopen(obj);
    % Set up the filters enable
    fwrite(obj,'y','char');
    replay = char(fread(obj,1,'char'));
    if strcmp(replay,'y')
        fwrite(obj,MIAEnable,'char');
        fwrite(obj,MIAEnable,'char');
        replay = char(fread(obj,1,'char'));
        if strcmp(replay,'y')
            if(MIAEnable)
                set(handles.t_msg,'String','MIA enabled');
            else
                set(handles.t_msg,'String','SENSOR-HAND enabled');
            end
        else
            set(handles.t_msg,'String','Error Setting MIA enable'); 
            fclose(obj);
            return
        end
    else
