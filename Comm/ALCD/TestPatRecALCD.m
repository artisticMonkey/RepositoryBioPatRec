...
function [accuracy, outLabels] = TestPatRecALCD(handles, sigFeatures, tSets, tOuts)
	...
	nFeatures    = size(handles.patRec.selFeatures,1);
...
%         end
    ...
    %% Transient compatible - DD 20190417 %
%     transTrueLabels = trueLabels;
%     count = 0;
%     for i = 1:(size(trueLabels,1)-size(trueLabels,1)/4)
%         transTrueLabels(i+count) = trueLabels(i);
%         transTrueLabels(i+count+1) = trueLabels(end);
%         count = count + 1;
%     end
    alg = handles.patRec.alg; % DD 20190417
    selFeatures = handles.patRec.selFeatures;
    patRec = handles.patRec;
%     if strcmp(alg,'SVMtHW')||strcmp(alg,'SVMt')
%         
%         [transSets, transOuts,patRec.threshold, patRec.maxVals, transTSets, transTOuts] = GetSets_Stack_TransientMov(sigFeatures, selFeatures, 0.25, patRec); %AM 20180921
%         
%         tSets = transTSets;
%         tOuts = transTOuts;
%         nWindows = size(tSets,1);
...
%     end
    ...
    replay = char(fread(obj,1,'char'));
    if strcmp(replay,'c')
        set(handles.t_msg,'String','Test of ALCD onboard PR completed');
    else
    ...
    confMat = confusionmat(trueLabels, outLabels) % KD: 20220523
...
%     maskMat = zeros(size(tSets,1),nM);
%     FN      = zeros(size(tSets,1),nM);
%     TN      = zeros(size(tSets,1),nM);
%     TP      = zeros(size(tSets,1),nM);
%     FP      = zeros(size(tSets,1),nM);
    confMatFlag = 1;
    
    if strcmp(alg,'SVMtHW')||strcmp(alg,'SVMt')
        sM      = size(tOuts,1)/(nM-1);          % Sets per movement
        maskMat = zeros(size(tSets,1),nM);
