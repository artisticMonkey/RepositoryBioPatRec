...
% 2022-04-12 / Katarina Dejanovic/ Changed the SVM function (from svmtrain
% to fitcsvm, and from svmclassify to predict)
...
%                              'boxconstraint',1);
    % KD: 20220412
    svmStruct{i} = fitcsvm(trSets,trGroup,'KernelFunction', dType,...
        'BoxConstraint',1);

    % Validate SVM
%     vClass = svmclassify(svmStruct(i),vSets);
    vClass = predict(svmStruct{i},vSets);
...
%                                     'kernel_function', dType,...
%                                      'boxconstraint',C(j));
                svmVal{j} = fitcsvm(trSets,trGroup,'KernelFunction',dType,...
                    'BoxConstraint',C(j));
%                 vClass = svmclassify(svmVal(j),vSets);
                vClass = predict(svmVal{i},vSets);
                vGood = find(vClass==vGroup);
...
%                 vClass = svmclassify(svmVal(idx),vSets);
                vClass = predict(svmVal{idx},vSets);
                vGood = find(vClass==vGroup);
                vAcc(j) = size(vGood,1) / nVal;
            end
            [~,bestidx] = max(vAcc);
...
%             vClass = svmclassify(svmStruct(i),vSets);
        ...
        elseif strcmp(dType,'polynomial')
            ...
            for j=1:numel(C)
                svmVal{j} = fitcsvm(trSets,trGroup,'KernelFunction',dType,...
                    'BoxConstraint',C(j),'PolynomialOrder',d(j));
%                 svmVal(j) = svmtrain(trSets,trGroup,...
%                                      'kernel_function', dType,...
%                                      'boxconstraint',C(j),...
%                                      'polyorder', d(j));
%                 vClass = svmclassify(svmVal(j),vSets);
                vClass = predict(svmVal{j},vSets);
                vGood = find(vClass==vGroup);
                vAcc(j) = size(vGood,1) / nVal;
            end
            [~,idx] = max(vAcc);
...
%             svmStruct(i) = svmVal(idx);
        ...
        elseif strcmp(dType,'rbf')
            ...
            for j=1:numel(C)
                svmVal{j} = fitcsvm(trSets,trGroup,'KernelFunction',dType,...
                    'BoxConstraint',C(j),'KernelScale',sigma(j));
%                 svmVal(j) = svmtrain(trSets,trGroup,...
%                                      'kernel_function', dType,...
%                                      'boxconstraint',C(j),...
%                                      'rbf_sigma', sigma(j));
