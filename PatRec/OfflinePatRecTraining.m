...
function patRec = OfflinePatRecTraining(alg, tType, algConf, trSets, trOuts, vSets, vOuts, mov, movIdx, transSets, transOuts, autoscale)
    ...
    % SVMt
    elseif strcmp(alg,'SVMt')
        
        [SVM accV] = SVMTransAnalysis(tType, transSets, transOuts, autoscale);
        patRec.algorithm = 'SVMt';
        patRec.training = tType;
        patRec.SVM = SVM; 

    % SVMcv
    elseif strcmp(alg,'SVMcv')
        
        [SVM accV] = SVMTransAnalysis(tType, transSets, transOuts, autoscale);
        patRec.algorithm = 'SVMcv';
        patRec.training = tType;
        patRec.SVM = SVM; 
        
    % SVMtHW
    elseif strcmp(alg,'SVMtHW')
        
        [SVM accV] = SVMTransAnalysisHW(tType, transSets, transOuts, mov, movIdx );
%         tType, trSets, trOuts, vSets, vOuts, mov, movIdx
        patRec.algorithm = 'SVMtHW';
        patRec.training = tType;
        patRec.SVM = SVM; 
    ...
    % ANNt
    elseif strcmp(alg,'ANNt')
        
        [ANN accV] = ANNTransAnalysis(tType, transSets, transOuts);
        patRec.algorithm = 'ANNt';
        patRec.training = tType;
        patRec.ANN = ANN; 

    %LDAt
    elseif strcmp(alg,'LDAt')
        
        [LDA accV] = LDATransAnalysis(tType, transSets, transOuts);
        patRec.algorithm = 'LDAt';
        patRec.training = tType;
        patRec.LDA = LDA; 
