...
% Last Modified by GUIDE v2.5 09-Feb-2023 12:48:41
    ...
    if ~isequal(file, 0)
        ...
        if(strcmp(ext,'.mat'))
            ...
            if (exist('recSession','var'))      % Send a recording session for data treatment
                ...
                set(handles.t_repetitionTime,'Value',recSession.rT + recSession.cT);
                ...
                set(handles.pb_RealtimePatRecGUI,'Enable','off');
...
% handles    structure with handles and user data (see GUIDATA)
    ...
    global onThrCoeff 
    global propRatio
    onThrCoeff = str2num(get(handles.et_onThr,'String'));
    propRatio = str2num(get(handles.et_propRatio,'String'));
...
%    set(handles.pb_RunOfflineTraining,'Enable','Off');    
    ...
    %Signal plot
    plotFlag = get(handles.cb_signalPlot,'Value');
    ...
    %Select transfer learning algorithm
    allTransAlg = get(handles.pm_TransferAlgorithm,'String');
    transAlg = char(allTransAlg(get(handles.pm_TransferAlgorithm,'Value')));
    %Select data extension method
    allDataExtension = get(handles.pm_data_extension,'String');
    dataExtension = char(allDataExtension(get(handles.pm_data_extension,'Value')));
    %Retrain the algorithm
    retrain = get(handles.cb_retrain,'Value');
    %Data for training and cross-validation
    CVFeatures.trainAll = get(handles.cb_trainAll,'Value');
    CVFeatures.testData = str2num(get(handles.et_CVdata,'String'));
    CVFeatures.repetitionTime = get(handles.t_repetitionTime,'Value'); % time it takes to finish one repetition of the movement plus rest phase (cT+rT)

    transLearningFeatures.transAlg = transAlg;
    transLearningFeatures.dataExtension = dataExtension;
    transLearningFeatures.retrain = retrain;
    
    % Call rutine for offline pat rec
    patRec = OfflinePatRec(sigFeatures, selFeatures, randFeatures, normSets, alg, tType, algConf, movMix, topology, confMatFlag, featReducAlg, plotFlag, transLearningFeatures, CVFeatures);
        
    % Save and show results
    
    % Shift minimum data from MAV to dMAV
    try
        patRec.normSets.minMovValues = handles.patRec.normSets.minMovValues;
    catch
    end
...
%    alg = get(hObject,'Value'); %returns pm_SelectAlgorithm contents as cell array
    ...
    elseif strcmp(alg,'SVMt') || strcmp(alg,'SVMcv') % for SVM-transient
        set(handles.pm_SelectTraining,'Enable','on');
        tA = {'Select Training A.','linear', 'quadratic', 'polynomial', 'rbf', 'mlp'};
        set(handles.pm_SelectTraining,'String',tA);
        set(handles.pm_SelectTraining,'Value',1);
    ...
    elseif strcmp(alg,'SVMtHW'); % for SVM-transient hand+wrist
        set(handles.pm_SelectTraining,'Enable','on');
        tA = {'Select Training A.','linear', 'quadratic', 'polynomial', 'rbf', 'mlp'};
        set(handles.pm_SelectTraining,'String',tA);
        set(handles.pm_SelectTraining,'Value',1);
...
if isfield(handles,'patRec')
    ...
    elseif strcmp(handles.patRec.patRecTrained.algorithm, 'SVM')
        UpdateCoefficientsALCD(handles);
    elseif strcmp(handles.patRec.patRecTrained.algorithm, 'SVMt')
        UpdateCoefficientsALCD(handles);
    elseif strcmp(handles.patRec.patRecTrained.algorithm, 'SVMtHW')
        UpdateCoefficientsALCD(handles);
    else
...
% --------------------------------------------------------------------
function m_ALC_test_sig_Callback(hObject, eventdata, handles)
% hObject    handle to m_ALC_test_feats (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
    set(handles.t_msg,'String','ALCD PR Signal Test Started...');  
    drawnow;
    ...
    if isempty(sigTreated)
        set(handles.t_msg,'String','No treated signal loaded!!!');
        return;
    end

    [accuracy outLabels] = TestSigPatRecALCD(handles, sigTreated);
    

% --------------------------------------------------------------------
function m_ALC_test_feats_Callback(hObject, eventdata, handles)
% hObject    handle to m_ALC_test_feats (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
    set(handles.t_msg,'String','ALCD PR Test Started...');  
    drawnow;
    ...
    %% Normalize (done in ALCD)
    %[trSets vSets patRec] = NormalizeSets_TrV(normSetsType, trSets, vSets, patRec);
    
    patRec = handles.patRec;
    if strcmp(alg,'SVMtHW')||strcmp(alg,'SVMt')
        
        [transSets, transOuts,patRec.threshold, patRec.maxVals, transTSets, transTOuts, patRec.normSets.minMovValues, ~, ~] = GetSets_Stack_TransientMov(sigFeatures, selFeatures, 0.25, patRec, plotFlag, patRec.propRatio); %AM 20180921
    ...
    % These are hardcoded
%     nFeatures = 4;
    nFeatures = size(patRec.selFeatures,1);
%     nCh = size(tSets,2)/nFeatures;
    nCh = length(sigFeatures.nCh); % KD: 20220519
    nMovs =  size(sigFeatures.mov,1);
    
    % Call rutine for offline pat rec
    [accuracy outLabels] = TestPatRecALCD(handles, sigFeatures, tSets, tOuts);
...
% --------------------------------------------------------------------
function m_ALC_onboard_Callback(hObject, eventdata, handles)
% hObject    handle to m_ALC_onboard (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)


% --------------------------------------------------------------------
function m_ALC_table_Callback(hObject, eventdata, handles)
% hObject    handle to m_ALC_table (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
...
%             if (exist('recSession','var') && isfield(recSession,'ramp'))
    ...
    drawnow;
    patRec = handles.patRec;
    propTable = CreatePRtable_ALCD(tW, iW, recSession, patRec);
    ...
    xlabel('TMABS','FontSize',20);
    set(gca, 'FontSize',16,'FontWeight','light');    
    % Create GUI-window
    ...
    uicontrol('Style', 'pushbutton', 'String', 'Confirm',...
        ...
        'Callback', {@UpdatePRtable,hObject,movValBoxes,propTable, handles, nCh, nM});
...
function UpdatePRtable(hObj, event, inObj, movValBoxes, propTable, handles, nCh, nM)
    ...
    handles.patRec.pTable = prostheticOutputs;
    propChannels = handles.patRec.patchPropChannels;
    ComMode     = handles.patRec.comm;
    Device      = handles.patRec.dev;
    propChannelsCompressed = CompressPropChannels(propChannels);
    if strcmp(Device, 'ADS_BP') || strcmp(Device, 'ALCD') || strcmp(Device, 'ALD-16chs') || strcmp(Device, 'ALC-24chs')
...
%             end
    ...
    % KD:20220526, send the channels which are used
    vCh = handles.patRec.vCh;
    ...
    for i = 1:nM
        for k = 1:nCh
            fwrite(obj,propTable(k,i),'float32'); 
        end
        fwrite(obj,propChannelsCompressed(i),'uint32'); % KD: 20220519, send channels which are relevant for each movement for proportional control
    end
    toc;
    replay = char(fread(obj,1,'char'));
    if strcmp(replay,'v');
...
% --------------------------------------------------------------------
function m_ALC_testControlChain_Callback(hObject, eventdata, handles)
% hObject    handle to m_ALC_testControlChain (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

    set(handles.t_msg,'String','ALCD Control-Chain Test Started...');  
    drawnow;
    ...
    if randFeatures
     ...
     elseif strcmp(movMix, 'Individual Mov')
         ...
         if strcmp(alg,'SVMt') % KD:20220526
             [trSets, trOuts, tSets, tOuts, movIdx, movOutIdx] = GetSets_Stack_IndvMov_Transient(sigFeatures, selFeatures, 0.25);
         else
             [trSets, trOuts, vSets, vOuts, tSets, tOuts, movIdx, movOutIdx] = GetSets_Stack_IndvMov(sigFeatures, selFeatures);
         end
    ...
    % These are hardcoded
    nFeatures = length(selFeatures);
    nCh = size(tSets,2)/nFeatures;
    nMovs =  size(sigFeatures.mov,1);
    
    % Call rutine for offline pat rec
    [strength outLabels] = TestControlChainALCD(handles, sigFeatures, tSets, tOuts);
    figure('Name','Proportional control')
    plot(strength)
    figure('Name','MAV')
    plot(mean(tSets(:,1:8),2))



function et_onThr_Callback(hObject, eventdata, handles)
% hObject    handle to et_onThr (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

% Hints: get(hObject,'String') returns contents of et_onThr as text
%        str2double(get(hObject,'String')) returns contents of et_onThr as a double


% --- Executes during object creation, after setting all properties.
function et_onThr_CreateFcn(hObject, eventdata, handles)
% hObject    handle to et_onThr (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    empty - handles not created until after all CreateFcns called
...
% --- Executes on button press in cb_derivativeOnset.
function cb_derivativeOnset_Callback(hObject, eventdata, handles)
% hObject    handle to cb_derivativeOnset (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
global derivativeOnset
global waitSamples

waitSamples = str2num(get(handles.et_waitSamples,'String')); 
derivativeOnset = get(handles.cb_derivativeOnset,'Value');

% Hint: get(hObject,'Value') returns toggle state of cb_derivativeOnset



function et_waitSamples_Callback(hObject, eventdata, handles)
% hObject    handle to et_waitSamples (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

% Hints: get(hObject,'String') returns contents of et_waitSamples as text
%        str2double(get(hObject,'String')) returns contents of et_waitSamples as a double


% --- Executes during object creation, after setting all properties.
function et_waitSamples_CreateFcn(hObject, eventdata, handles)
% hObject    handle to et_waitSamples (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    empty - handles not created until after all CreateFcns called
...
% --- Executes on button press in cb_signalPlot.
function cb_signalPlot_Callback(hObject, eventdata, handles)
% hObject    handle to cb_signalPlot (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

% Hint: get(hObject,'Value') returns toggle state of cb_signalPlot


% --- Executes on selection change in pm_TransferAlgorithm.
function pm_TransferAlgorithm_Callback(hObject, eventdata, handles)
% hObject    handle to pm_TransferAlgorithm (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

% Hints: contents = cellstr(get(hObject,'String')) returns pm_TransferAlgorithm contents as cell array
%        contents{get(hObject,'Value')} returns selected item from pm_TransferAlgorithm


% --- Executes during object creation, after setting all properties.
function pm_TransferAlgorithm_CreateFcn(hObject, eventdata, handles)
% hObject    handle to pm_TransferAlgorithm (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    empty - handles not created until after all CreateFcns called
...
% --- Executes on selection change in pm_data_extension.
function pm_data_extension_Callback(hObject, eventdata, handles)
% hObject    handle to pm_data_extension (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

% Hints: contents = cellstr(get(hObject,'String')) returns pm_data_extension contents as cell array
%        contents{get(hObject,'Value')} returns selected item from pm_data_extension


% --- Executes during object creation, after setting all properties.
function pm_data_extension_CreateFcn(hObject, eventdata, handles)
% hObject    handle to pm_data_extension (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    empty - handles not created until after all CreateFcns called
...
% --- Executes on button press in cb_retrain.
function cb_retrain_Callback(hObject, eventdata, handles)
% hObject    handle to cb_retrain (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

% Hint: get(hObject,'Value') returns toggle state of cb_retrain



function et_propRatio_Callback(hObject, eventdata, handles)
% hObject    handle to et_propRatio (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

% Hints: get(hObject,'String') returns contents of et_propRatio as text
%        str2double(get(hObject,'String')) returns contents of et_propRatio as a double


% --- Executes during object creation, after setting all properties.
function et_propRatio_CreateFcn(hObject, eventdata, handles)
% hObject    handle to et_propRatio (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    empty - handles not created until after all CreateFcns called
...
% --- Executes on button press in cb_trainAll.
function cb_trainAll_Callback(hObject, eventdata, handles)
% hObject    handle to cb_trainAll (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

% Hint: get(hObject,'Value') returns toggle state of cb_trainAll



function et_CVdata_Callback(hObject, eventdata, handles)
% hObject    handle to et_CVdata (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

% Hints: get(hObject,'String') returns contents of et_CVdata as text
%        str2double(get(hObject,'String')) returns contents of et_CVdata as a double


% --- Executes during object creation, after setting all properties.
function et_CVdata_CreateFcn(hObject, eventdata, handles)
% hObject    handle to et_CVdata (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    empty - handles not created until after all CreateFcns called
...
% --- Executes on button press in pb_GetFeaturesMagnets.
function pb_GetFeaturesMagnets_Callback(hObject, eventdata, handles)
% hObject    handle to pb_GetFeaturesMagnets (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
disp('%%%%%%%%%%% Loading Data %%%%%%%%%%%%%');
set(handles.t_msg,'String','Loading Data...');
...
% Dialog box to open a file
[file, path] = uigetfile({'*.mat';'*.csv';'*.txt'},'Multiselect','on');
set(handles.t_fileName,'String',file);
% Check that the loaded file is a "ss" struct
if ~isequal(file, 0)
    [pathstr,name,ext] = fileparts(file);
    if (strcmp(ext,'.mat'))
        ...
        if (exist('recSession','var'))      % Send a recording session for data treatment
            ...
            Load_recSessionMagnets(recSession, handles);
            if isfield(recSession,'vCh')
                handles.vCh = recSession.vCh;
            end     
            set(handles.t_repetitionTime,'Value',recSession.rT + recSession.cT);
