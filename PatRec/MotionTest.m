...
sF                  = motionTest.sF;
if isfield(patRec,'vCh')
    handles.vCh = patRec.vCh;
end
...
% Allocate resources for additional sensor data
if isfield(patRec,'useAcceleGlove') && patRec.useAcceleGlove == 1
    useAcceleGlove = 1;
    sensorData.accelerometers = zeros(18,sT/tW);
    ag_port = patRec.acceleGlovePort;
else
    useAcceleGlove = 0;
...
%                 s = InitSBI_NI(sF,sT,sCh);
                ...
                % Start DAQ
                s.startBackground();                                               % Run in the background
