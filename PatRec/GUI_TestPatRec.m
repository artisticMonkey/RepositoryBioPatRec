...
% Last Modified by GUIDE v2.5 07-Mar-2012 16:35:05
...
if get(handles.cb_VRE,'Value')
    global TAC
    %Calculate dof, since every 2nd value indicates which dof. (works for
    %now)
    dof = round(motorDir/2);
    dof = 10;
    %Calculate which direction to move in.
    dir = mod(motorDir,2);
    %Moves the VRE some distance.
    if VREActivation(handles.vre_Com,movSpeed,movTime,dof,dir,get(handles.cb_moveTAC,'Value'));
        %TAC is complete.
...
if get(handles.cb_VRE,'Value')
    ...
    %Calculate which direction to move in.
    dir = mod(motorDir,2);
    %Moves the VRE a short distance.
    VREActivation(handles.vre_Com,movSpeed,movTime,dof,dir,get(handles.cb_moveTAC,'Value'));
end
if get(handles.cb_motorCoupling,'Value')
    % Get the communication obj
    com = handles.com;
...
set(handles.cb_VRE,'Enable','off');
set(handles.pb_Camera,'Enable','off');
set(handles.cb_VRE,'Value',0)
obj = handles.vre_Com;
fclose(obj);
...
% --- Executes on button press in cb_VRE.
function cb_VRE_Callback(hObject, eventdata, handles)
% hObject    handle to cb_VRE (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

% Hint: get(hObject,'Value') returns toggle state of cb_VRE
...
if TAC.running
    ...
    t = toc;
    set(handles.t_timeTAC,'String',sprintf('Time:%0.3f s',t));
    time = TAC.timer;
    stop(time);
else
    TAC.running = 1;
    ...
    time = timer('Period',0.05,'ExecutionMode','fixedRate');
    time.TimerFcn = {@callback, handles};
    start(time);
    TAC.timer = time;
    TAC.ackTimes = 0;
end
...
% --- Executes on button press in cb_showTAC.
function cb_showTAC_Callback(hObject, eventdata, handles)
% hObject    handle to cb_showTAC (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
if get(hObject,'Value') %Showing hand!
    set(handles.pb_startTAC,'Enable','on');
    set(handles.cb_moveTAC,'Enable','on');
else
    set(handles.pb_startTAC,'Enable','off');
    set(handles.cb_moveTAC,'Enable','off');
    set(handles.cb_moveTAC,'Value',0);
end
if isfield(handles,'vre_Com')
    obj = handles.vre_Com;
    fwrite(obj,sprintf('%c%c%c%c','c',char(2),char(0),char(0)));
    fread(obj,1);
end
