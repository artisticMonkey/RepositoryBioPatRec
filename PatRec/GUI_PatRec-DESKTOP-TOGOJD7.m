...
% Last Modified by GUIDE v2.5 13-May-2022 17:36:31
...
%    set(handles.pb_RunOfflineTraining,'Enable','Off');    
    ...
    patRec = OfflinePatRec(sigFeatures, selFeatures, randFeatures, normSets, alg, tType, algConf, movMix, topology, confMatFlag, featReducAlg, plotFlag, transAlg, dataExtension, retrain);
...
%    alg = get(hObject,'Value'); %returns pm_SelectAlgorithm contents as cell array
    ...
    elseif strcmp(alg,'SVMt'); % for SVM-transient
        set(handles.pm_SelectTraining,'Enable','on');
        tA = {'Select Training A.','linear', 'quadratic', 'polynomial', 'rbf', 'mlp'};
        set(handles.pm_SelectTraining,'String',tA);
        set(handles.pm_SelectTraining,'Value',1);
...
% handles    structure with handles and user data (see GUIDATA)
    ...
    if strcmp(alg,'SVMtHW')||strcmp(alg,'SVMt')
        ...
        [transSets, transOuts,patRec.threshold, patRec.maxVals, transTSets, transTOuts, patRec.normSets.minMovValues, patchPropChannels, proportionalMethodDifference] = GetSets_Stack_TransientMov(sigFeatures, selFeatures, 0.25, patRec, plotFlag, patRec.propRatio); %AM 20180921
...
function UpdatePRtable(hObj, event, inObj, movValBoxes, propTable, handles, nCh, nM)
    ...
    for i = 1:nM
        for k = 1:nCh
            ...
            fwrite(obj,propChannels(i,k),'char'); % KD: 20220519
        end
    end
    toc;
    replay = char(fread(obj,1,'char'));
