...
if strcmp(patRec.alg,'SVMtHW')||strcmp(patRec.alg,'SVMt')||strcmp(patRec.alg,'ANNt')||strcmp(patRec.alg,'LDAt')
    nM      = size(patRec.mov,1)-1;       % Number of movements (total)
else
...
prediction = zeros(size(tOut,1),2);
% prediction metrics
...
for i = 1 : size(tSet,1)
    ...
    prediction(i,1) = find(tOut(i,:));
    prediction(i,2) = outMov;
    if (strcmp(patRec.alg,'SVMtHW') & outMov==(nOut+1))  % AM 20190116
        continue;
    end
...
%     e = sM*i;
    idx = find(tOut(:,i));
    sM(i) = sum(tOut(:,i) == 1);
    acc(i) = sum(maskMat(idx,i))/sM(i);
    %acc(i) = sum(good(s:e))/sM;
%     tPvec(:,i)=sum(TP(s:e,:));
%     tNvec(:,i)=sum(TN(s:e,:));
%     fPvec(:,i)=sum(FP(s:e,:));
%     fNvec(:,i)=sum(FN(s:e,:));
    tPvec(i,:)=sum(TP(idx,:),1);
    tNvec(i,:)=sum(TN(idx,:),1);
    fPvec(i,:)=sum(FP(idx,:),1);
    fNvec(i,:)=sum(FN(idx,:),1);

    if tPvec(:,i) > sM(i)
        disp('Error on True Possitives');
    end        
    if tNvec(:,i) > size(tSet,1)-sM(i)
        disp('Error on True Negatives');
    end    
...
% if confMatFlag
%     confMat(:,1:nM) = confMat(:,1:nM) ./ sM;% This will only work if there is an equal number of sets per class
%     figure;
%     imagesc(confMat);
