...
function joyStickTest = JoyStickTest(patRecX, handlesX)
...
global target2
global anim
global x y r xt yt rt xi yi ri
...
global tdist
global overshoot
global nOvershoot   % counts the number of times target is acquired then lost
...
%% Init joyStickTest structure
joyStickTest.patRec   = patRec;
joyStickTest.sF       = patRec.sF;
joyStickTest.trials   = trials;
joyStickTest.timeOut  = timeOut;
joyStickTest.nR       = nR;     % number of repetitions
joyStickTest.nIDs     = nIDs;    % number of Idexes of Difficulty
    ...
    for rep = 1:nR 
        ...
        for ID = 1:nIDs
           ...
           %% Target Randomization
            ...
            elseif disttype == 2
                if ~exist('count','var') || count == nR
                    count = 0;               % Reset count when it reaches num of reps (end of the list of combos)
                ...
                if ID == 1                   % target location index (count) is set to                                 
                    count = rep;             % the repetition number. That way, each target
                end                          % direction will be presented at each ID.
                xt = xi + combo(count,1).*targetDist;
                yt = yi + combo(count,2).*targetDist;
                rt = ri + combo(count,3).*targetDist;
...
%             test.indDiff                 = ID;
            ...
            disp(test);
            joyStickTest.test(rp(ID),rep,t)  = test;         % rp(ID) puts in order of ID set   
        end
    end
end
...
joyStickTest.distance = D; % save distance and width values.. Order may be different
joyStickTest.width = W;
joyStickTest.dwellTime              = dwellTime;
% organize and save performance metrics
joyStickTest = FittsTestMetrics(joyStickTest);  
...
[filename, pathname] = uiputfile({'*.mat','MAT-files (*.mat)'},'Save as', 'Untitled.mat');
    if isequal(filename,0) || isequal(pathname,0)
       disp('User pressed cancel')
        ...
        save([pathname,filename],'joyStickTest');    
    end
...
function JoyStickTest_OneShot(src,event)
...
%     global nwCompTime;  % Number of time windows to consider completion time
    ...
    global success
    global overshoot
    global nOvershoot   % counts the number of times target is acquired then lost (overshot)
    global dwelltWs
    global targetWidth
    global expand
    global shrink
...
%             end
   ...
   % gets speeds and map to [0,100] interval
   speeds = (patRec.control.currentDegPerMov./patRec.control.maxDegPerMov).*100;
   % get speed of specific movement class
   speed = speeds(outMov);
 ...
 %%%%%%%%% ****************** %%%%%%%%%%%%   
         if outMov == nM
            ...
            anim.XData = x+r*cos(p);
            anim.YData = y+r*sin(p);
         ...
         if [([x,y,r] <= [xt,yt,rt] + targetWidth/2), ([x,y,r] >= [xt,yt,rt] - targetWidth/2)]
             ...
             anim.Color = 'blue';
             if overshoot == 1
                 nOvershoot = nOvershoot + 1;        % counts number of times target was acquired then lost
             end
             overshoot = 0;
         end
         drawnow
