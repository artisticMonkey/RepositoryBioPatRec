...
function [movMatrix, movMatrixNorm] = CreateMovMatrix(patRec)
    ...
    nMovDims  = floor(nIndvMovs/2)+1;   % DD 09/09/2019 - Lateral and Pinch grip are 2 different DoFs
%     nMovDims  = floor(nIndvMovs/2);
    ...
    movMatrix(end-1,end-1) = 0;   % Lateral and Side grip are not coupled movements
    movMatrix(end-2,end-1) = 1;   % Lateral and Side grip are not coupled movements
    movMatrix(end-1,end) = 1;   % Lateral and Side grip are not coupled movements
