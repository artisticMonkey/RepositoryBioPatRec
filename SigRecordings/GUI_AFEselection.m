...
% Last Modified by GUIDE v2.5 07-Oct-2022 15:30:07
...
%% Compression (only for ADS_BP and ALC-D 1.2)
AFE_settings.compressData = get(handles.cb_compressionEnable,'Value');
...
%useLeg = handles.varargin{11};
replaceRec = handles.varargin{12}; % JF 20191023
...
AFE_settings.useAcceleGlove = get(handles.cb_acceleGlove,'Value');
AFE_settings.acceleGlovePort = get(handles.et_AGPort, 'String');
...
%                         [cdata, sF, sdata] =
%                         RecordingSession(nM,nR,cT,rT,mov,hGUI_Rec,AFE_settings,get(handles.cb_trainVRE,'Value'),vreMovements,get(handles.cb_VRELeftHand,'Value'),movRepeatDlg,useLeg,rampStatus,rampParams); 
            [cdata, sF, sdata] = RecordingSession(nM,nR,cT,rT,mov,hGUI_Rec,AFE_settings,get(handles.cb_trainVRE,'Value'),vreMovements,get(handles.cb_VRELeftHand,'Value'),movRepeatDlg,useLeg,rampStatus,rampParams,replaceRec);%Fs,Ne,Nr,Tc,Tr,Psr,msg,EMG_AQhandle,rampParams); Added replaceRec JF 20191023
        else
%             [cdata, sF, sdata] = RecordingSession(nM,nR,cT,rT,mov,hGUI_Rec,AFE_settings,get(handles.cb_trainVRE,'Value'),vreMovements,get(handles.cb_VRELeftHand,'Value'),movRepeatDlg,useLeg,rampStatus);%Fs,Ne,Nr,Tc,Tr,Psr,msg,EMG_AQhandle);            [cdata, sF, sdata] = RecordingSession(nM,nR,cT,rT,mov,hGUI_Rec,AFE_settings,get(handles.cb_trainVRE,'Value'),vreMovements,get(handles.cb_VRELeftHand,'Value'),movRepeatDlg,useLeg,rampStatus);%Fs,Ne,Nr,Tc,Tr,Psr,msg,EMG_AQhandle);
            [cdata, sF, sdata] = RecordingSession(nM,nR,cT,rT,mov,hGUI_Rec,AFE_settings,get(handles.cb_trainVRE,'Value'),vreMovements,get(handles.cb_VRELeftHand,'Value'),movRepeatDlg,useLeg,rampStatus,replaceRec);%Fs,Ne,Nr,Tc,Tr,Psr,msg,EMG_AQhandle); Added replaceRec JF 20191023
        end
        ...
        tempdata = cdata;
        save('cdata.mat','cdata','tempdata','sdata','sF','sT','nCh','ComPortType','deviceName');
        close(GUI_AFEselection);
%         close(GUI_RecordingSession);
%         close(GUI_Recordings);
    ...
    case 1
        ...
        [cdata, sF, sT, sdata] = FastRecordingSession(hGUI_Rec,AFE_settings);
...
if strcmp(deviceName, 'ALC-16chs')
    ...
    handles.vCh = vCh;
    set(handles.et_chs,'String',num2str(handles.nCh));
    set(handles.ComPortType,'Value',3);
...
deviceNames = {'AD2','AD3','MyoAmpF4F1','MyoAmpF4F2','MyoAmpF4F7','MyoAmpF4F7_Shielded','NeuroAmpF8F1','RHA2216','RHA2132','ADS1299','ADS_BP','Thalmic MyoBand','ALC-16chs','ALC-24chs'};
...
% --- Executes on button press in cb_acceleGlove.
function cb_acceleGlove_Callback(hObject, eventdata, handles)
% hObject    handle to cb_acceleGlove (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
if get(handles.cb_acceleGlove,'Value') == 1
    set(handles.et_AGPort, 'Visible', 'on');
    set(handles.lb_AGPort, 'Visible', 'on');
else
    set(handles.et_AGPort, 'Visible', 'off');
    set(handles.lb_AGPort, 'Visible', 'off');
end
% Hint: get(hObject,'Value') returns toggle state of cb_acceleGlove



function et_AGPort_Callback(hObject, eventdata, handles)
% hObject    handle to et_AGPort (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

% Hints: get(hObject,'String') returns contents of et_AGPort as text
%        str2double(get(hObject,'String')) returns contents of et_AGPort as a double


% --- Executes during object creation, after setting all properties.
function et_AGPort_CreateFcn(hObject, eventdata, handles)
% hObject    handle to et_AGPort (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    empty - handles not created until after all CreateFcns called
...
% --- Executes on button press in cb_compressionEnable.
function cb_compressionEnable_Callback(hObject, eventdata, handles)
% hObject    handle to cb_compressionEnable (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

% Hint: get(hObject,'Value') returns toggle state of cb_compressionEnable
