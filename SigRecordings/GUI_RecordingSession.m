...
% Last Modified by GUIDE v2.5 23-Oct-2019 12:51:34
...
function pb_Record_Callback(hObject, eventdata, handles)
    ...
    hGUI_Rec = guidata(h1);
    rampStatus = get(handles.pm_trainingCurve,'Value')-1;
    movRepeatDlg = get(handles.cb_movRepeatDlg,'Value');
    replaceRec = get(handles.cb_repeat,'Value');       %%%%%% JF 20191023
...
%     GUI_AFEselection(nM,nR,cT,rT,mov,hGUI_Rec,vreMovements,rampStatus,fast,movRepeatDlg, useLeg)
    GUI_AFEselection(nM,nR,cT,rT,mov,hGUI_Rec,vreMovements,rampStatus,fast,movRepeatDlg, useLeg, replaceRec) % added replaceRec JF 20191023
...
% --- Executes on selection change in pm_trainingCurve.
function pm_trainingCurve_Callback(hObject, eventdata, handles)
% hObject    handle to pm_trainingCurve (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

% Hints: contents = cellstr(get(hObject,'String')) returns pm_trainingCurve contents as cell array
%        contents{get(hObject,'Value')} returns selected item from pm_trainingCurve


% --- Executes during object creation, after setting all properties.
function pm_trainingCurve_CreateFcn(hObject, eventdata, handles)
% hObject    handle to pm_trainingCurve (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    empty - handles not created until after all CreateFcns called
...
% --- Executes on button press in cb_repeat.
function cb_repeat_Callback(hObject, eventdata, handles)
% hObject    handle to cb_repeat (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

% Hint: get(hObject,'Value') returns toggle state of cb_repeat
