...
function [cdata, sF, sensorData] = RecordingSession(varargin)
...
replaceRec = varargin{end};  % JF 20191023
saveRec     = true;
...
if replaceRec == 1 % JF 20191023
        [file,path] = uigetfile('.mat','Select file to replace selected movement recordings');
        fullname = fullfile(path, file);
        replacing = load(fullname);
        
        nR = replacing.recSession.nR;     % number of excersice repetition
        cT = replacing.recSession.cT;     % time that the contractions should last
        rT = replacing.recSession.rT;     % relaxing time
        sF = replacing.recSession.sF;     % sampling frequency
        nCh = replacing.recSession.nCh;
...
if rampStatus
    ...
    if rampStatus == 1
        ...
        %         p_effortPlot = plot(handles.a_effortPlot,linspace(0,cT,sF*cT),linspace(50,50,sF*cT),'LineWidth',2);   % DD - 20191021
        gaussCurve = gauss2mf(linspace(0,cT,sF*cT),[0.13 cT/2.3 0.25 cT/2.3]);
        p_effortPlot = plot(handles.a_effortPlot,linspace(0,cT,sF*cT),gaussCurve*55,'LineWidth',2);   % DD - 20200803
        xlim(handles.a_effortPlot, [0 cT]);
        ylim(handles.a_effortPlot, [0 100]);
    else
        p_effortPlot = plot(handles.a_effortPlot,[linspace(0,3,sF*3) linspace(3,5,sF*2) linspace(5,8,sF*3)],[linspace(0,60,sF*3) linspace(60,60,sF*2) linspace(60,0,sF*3)],'LineWidth',2);
        xlim(handles.a_effortPlot, [0 8]);
        cT = 8;
        sTall         = (cT+rT)*nR;
        sT            = cT+rT;
        handles.cT    = cT;
...
while ex <= nM
    ...
    if strcmp (ComPortType, 'NI')
        ...
        for timeWindowNr = 1:0.4/tW
            [~, error] = Acquire_tWs(deviceName, obj, nCh, tWs, compressData);
        end
        ...
        for rep = 1 : nR
            ...
            UpdateGUI = 1;
            cData = zeros(tWs, nCh);
            if (get(handles.t_stopAcq,'Value'))
                break;
            end
            tic
            for timeWindowNr = 1:sT/tW
                ...
                [cData, error] = Acquire_tWs(deviceName, obj, nCh, tWs, compressData);    % acquire a new time window of samples
...
if saveRec                                                             % Saved in DataAnalysis
    ...
    recSession.useAcceleGlove = useAcceleGlove;
    if useAcceleGlove
        recSession.sensorData = sensorData;
        recSession.ag_port = ag_port;
    end
    recSession.mov      = mov;
    ...
    if replaceRec == 1 %JF 20191023
        s = size(replacing.recSession.cmt);
        replacing.recSession.cmt(s(1,1)+1,1)=recSession.cmt;
        for k = 1:length(recSession.mov)
            fstr = strcmpi(recSession.mov(k,1),replacing.recSession.mov);
            f = find(fstr==1);
            replacing.recSession.tdata(:,:,f) = recSessionData(:,:,k);
            replacing.recSession.cmt(s(1,1)+1,k+1)=recSession.mov(k,1);
        end
        
        
        [filename, pathname] = uiputfile({'*.mat','MAT-files (*.mat)'},'Save as', file);
        recSession = replacing.recSession;
