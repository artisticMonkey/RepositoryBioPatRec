...
%
% ------------- Update --------------
% 20/08/2021 -> Adding different normalization, per subject

function [transientSet, transientOut, threshold, maxVals, NoiseMAV] = GetSets_Stack_TransientMov_Luis_CrossSubject(sigFeatures, features, transLength, patRec, plotFlag, varargin)
...
clear tempIdx
try
    numSubjects = length(sigFeatures.nDataPerSubject); % number of subjects
    nDataPerSubject = sigFeatures.nDataPerSubject;  % number of data per subject per movement
    nDataPerMovement = sum(nDataPerSubject);    % number of data per movement of all subjects
    maxValue = [];
    minValue = [];
    rangeValue = [];
    midRangeValue = [];
    ...
    for subject = 1:numSubjects
        tempIdx = [];
        % obtain the indeces within transientSet variable, which are of the same subject
        for movement = 1:nMi
            if (isempty(varargin))
                tempIdx = [tempIdx, (1:nDataPerSubject(subject)) + nDataPerSubject(subject)*(subject - 1) + (movement - 1)*nDataPerMovement];
            else
                tempIdx = [tempIdx, (1:nDataPerSubject(subject)) + sum(nDataPerSubject(1:(subject - 1))) + (movement - 1)*nDataPerMovement(1)];
            end
        end
        subjectSet = transientSet(tempIdx,:); % a temporary variable which contain all the data of one subject
        % Normalization within range [-1,1]
        maxValue(subject,:) = max(subjectSet);
        minValue(subject,:) = min(subjectSet);
        rangeValue(subject,:) = maxValue(subject,:) - minValue(subject,:);
        midRangeValue(subject,:) = (maxValue(subject,:) + minValue(subject,:))/2;
        %data_set = (data_set - mMidrange) ./ (mRange/2);
        subjectSet = (subjectSet - midRangeValue(subject,:)) ./ (rangeValue(subject,:)/2);
        transientSet(tempIdx,:) = subjectSet;
    end
