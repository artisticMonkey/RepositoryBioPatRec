...
%     sampXmov = fix(totSamp / sigTreated.nR);    
%     sd = totSamp - sampXmov * sigTreated.nR; 
...
%     end
    ...
    trDataExt = trData;
    if (fs > size(trData,1))
        for i = 1:round(fs/size(trData,1))
            trDataExt = cat(1,trDataExt,trData);
        end
    end
    for ex = 1 : nM
        restData = [restData ; trDataExt(is:fs,:,ex)];         
    end
